import { Cpu, Zap, HardDrive, MemoryStick, Eye, ShoppingCart, X, Loader2, Settings, ChevronRight, Star, Check } from "lucide-react";
...
export const UnitesCentralesGaming = () => {
  ...
  const fetchGamingPCs = async () => {
    try {
      ...
      const caseProducts = allProducts.filter(
        (product: Product) => 
          product.reference && 
          product.reference.startsWith('CASE-') && 
          product.actif === true
      );
      
      console.log("Produits CASE- trouvés:", caseProducts);
      setProducts(caseProducts);
    } catch (error: any) {
      ...
      <section className="py-8 bg-secondary/20">
        <div className="container-x">
          <div className="flex items-center gap-2 mb-5">
            ...
            <h2 className="text-lg font-bold">Unités Centrales Gaming</h2>
          </div>
        ...
        <div className="container-x">
          ...
          <div className="bg-yellow-500/10 border border-yellow-500 rounded-lg p-6 text-center">
            <p className="text-yellow-600 dark:text-yellow-400 text-sm">
              Aucune unité centrale disponible pour le moment.
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="container-x">
          ...
          <div className="flex items-center justify-between mb-5">
            <div className="flex items-center gap-2">
              <div className="h-7 w-7 rounded-lg bg-gradient-to-br from-purple-500 to-indigo-500 flex items-center justify-center">
                <Cpu className="h-3.5 w-3.5 text-white" />
              </div>
              <div>
                <h2 className="text-base font-bold">Unités Centrales Gaming</h2>
                <p className="text-[10px] text-muted-foreground">Puissance et performance pour vos jeux</p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
            {products.map((product, index) => {
              ...
              return (
                ...
                >
                  {/* Image */}
                  <div className="relative aspect-square overflow-hidden bg-gradient-to-br from-purple-500/5 to-secondary/30">
                    {imageUrl ? (
                      <img 
                        src={imageUrl} 
                        alt={product.nom} 
                      ...
                      <div className="w-full h-full flex items-center justify-center">
                        <Cpu className="h-8 w-8 text-purple-500/20" />
                      </div>
                    )}
                  ...
                  <div className="p-2.5 space-y-1.5">
                    ...
                    <p className="text-[9px] text-muted-foreground line-clamp-2 leading-relaxed">
                      {product.description_courte || product.description?.substring(0, 60) || "PC gaming haute performance"}
                    </p>
                    ...
                    <div className="flex flex-wrap gap-1 pt-0.5">
                      <span className="text-[8px] bg-secondary/50 px-1.5 py-0.5 rounded-full text-muted-foreground flex items-center gap-0.5">
                        <Zap className="h-2 w-2" /> Gaming
                      </span>
                      {configurations.length > 0 && (
                        <span className="text-[8px] bg-secondary/50 px-1.5 py-0.5 rounded-full text-muted-foreground flex items-center gap-0.5">
                          <Settings className="h-2 w-2" /> {configurations.length} configs
                        </span>
                      )}
                    </div>
            ...
            >
                ...
                <div className="space-y-3">
                  ...
                  <div className="aspect-video bg-secondary/30 rounded-lg overflow-hidden">
                    {getImageUrl(selectedProduct) ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center">
                        <Cpu className="h-12 w-12 text-purple-500/30" />
                      </div>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="bg-secondary/30 rounded-lg p-2">
                      <h4 className="font-semibold text-[10px] text-purple-600 mb-1.5 uppercase tracking-wider">Informations</h4>
                      <div className="space-y-1 text-[10px]">
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">Référence :</span>
                          <span className="font-medium font-mono">{selectedProduct.reference}</span>
                        </div>
                        <div className="flex justify-between">
                    ...
                    <div className="bg-secondary/30 rounded-lg p-2">
                      <h4 className="font-semibold text-[10px] text-purple-600 mb-1.5 uppercase tracking-wider">Caractéristiques</h4>
                      <div className="space-y-1 text-[10px]">
                        <div className="flex justify-between">
                  ...
                  {isLoadingConfigs ? (
                    ...
                    <div className="bg-purple-500/5 rounded-lg p-3 border border-purple-500/10">
                      <div className="flex items-center gap-1.5 mb-2">
                        ...
                        <h4 className="font-semibold text-[10px] text-purple-600 uppercase tracking-wider">Configurations disponibles</h4>
                      </div>
                      <div className="space-y-1.5">
                        {configurations.map((config) => (
                          <div key={config.id} className="flex justify-between items-center text-[10px] border-b border-border/30 pb-1 last:border-0">
                            ...
                            <span className="font-bold text-purple-600">{formatPrice(config.prix_total, config.devise)}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  {/* Description */}
                  <div className="bg-purple-500/5 rounded-lg p-3 border border-purple-500/10">
                    <h4 className="font-semibold text-[10px] text-purple-600 mb-1.5 uppercase tracking-wider">Description</h4>
              ...
              <div className="sticky bottom-0 bg-background border-t border-border/50 p-3 flex gap-2">
                <div className="flex-1">
                  ...
                  <div className="font-bold text-sm text-purple-600">{formatPrice(selectedProduct.prix, selectedProduct.devise)}</div>
                </div>
                <button 
                  onClick={closeModal} 
