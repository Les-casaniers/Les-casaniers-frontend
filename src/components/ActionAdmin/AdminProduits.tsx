...
import {
  Package, PlusCircle, X, Save, Pencil, Trash2, Search, Eye, ChevronDown, ChevronRight, Grid, List, FolderTree, Layers, Tag, DollarSign, Warehouse, Image as ImageIcon, AlertCircle, CheckCircle, Zap, Box, Hash, Award, FileText, Plus, Loader2, Filter,
} from "lucide-react";
import {
  useProducts, useCreateProduct, useUpdateProduct, useDeleteProduct, useCategories, useSousCategories, useCreateSousCategory, useUpdateSousCategory, useDeleteSousCategory, SousCategory, useProductImageActions, Product as APIProduct, ProductFilters,
...
const ProductForm = ({
  ...
  const handleAddCaracteristique = () => {
    ...
    if (setCaracteristiques) {
      setCaracteristiques((prev) => ({ ...prev, [newCaractNom.trim()]: newCaractValeur.trim() }));
    }
    setNewCaractNom(""); setNewCaractValeur(""); setNewCaractType("texte"); setNewCaractObligatoire(false); setShowAddCaract(false);
  };
  ...
  const handleCaractValueChange = (nomChamp: string, value: string) => {
    if (setCaracteristiques) {
      setCaracteristiques((prev) => ({ ...prev, [nomChamp]: value }));
    }
  };

  const startEditingCaract = (nomChamp: string) => {
    setEditingCaract(nomChamp); setEditingTemplateId(null); setIsEditingTemplate(false);
    setNewCaractNom(nomChamp); setNewCaractValeur(caracteristiques?.[nomChamp] || "");
  };

  const startEditingTemplate = (template: TemplateCaracteristique) => {
    setEditingTemplateId(template.id); setEditingCaract(template.nom_champ); setIsEditingTemplate(true);
    setNewCaractNom(template.nom_champ); setNewCaractValeur(caracteristiques?.[template.nom_champ] || "");
  };
  ...
  const saveEditingCaract = () => {
    ...
    setEditingCaract(null); setEditingTemplateId(null); setIsEditingTemplate(false); setNewCaractNom(""); setNewCaractValeur("");
  };
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-1.5">
        <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-1.5"><Hash className="h-3.5 w-3.5" /> Type de référence</label>
        <select className={INPUT} value={selectedPrefix} onChange={(e) => { setSelectedPrefix(e.target.value); if (!isEditMode) generateReference(e.target.value); }} disabled={isEditMode}>
          <option value="">Sélectionner un type</option>
          {REFERENCE_PREFIXES.map((p) => (<option key={p.key} value={p.key}>{p.label} — {p.description}</option>))}
        </select>
      ...
      <div className="space-y-1.5">
        <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-1.5"><Package className="h-3.5 w-3.5" /> Nom du produit</label>
        <input className={INPUT} placeholder="Ex: RTX 4070 Ti Super 16GB" value={value.nom} onChange={(e) => setValue((p) => ({ ...p, nom: e.target.value }))} />
      </div>
      ...
      <div className="space-y-3 border-t border-border/50 pt-4">
        ...
        {templatesDisponibles.map((template) => {
          ...
          if (isEditing) {
            return (
              <div key={template.id} className="flex items-center gap-3 p-3 rounded-xl bg-primary/10 border border-primary/30">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-1">
                    <input className="px-3 py-1.5 text-sm border border-border/40 rounded-lg bg-background/60 focus:border-primary/60 focus:ring-2 focus:ring-primary/15 outline-none transition-all" placeholder="Nom du champ" value={newCaractNom} onChange={(e) => setNewCaractNom(e.target.value)} />
                    <input className="px-3 py-1.5 text-sm border border-border/40 rounded-lg bg-background/60 focus:border-primary/60 focus:ring-2 focus:ring-primary/15 outline-none transition-all" placeholder="Valeur par défaut" value={newCaractValeur} onChange={(e) => setNewCaractValeur(e.target.value)} />
                  </div>
                </div>
                <div className="flex gap-1.5">
                  <button type="button" onClick={saveEditingTemplate} disabled={!newCaractNom.trim()} className="p-1.5 rounded-lg bg-emerald-500/10 text-emerald-600 hover:bg-emerald-500/20 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"><CheckCircle className="h-4 w-4" /></button>
                  <button type="button" onClick={cancelEditing} className="p-1.5 rounded-lg bg-red-500/10 text-red-600 hover:bg-red-500/20 transition-colors"><X className="h-4 w-4" /></button>
                </div>
              </div>
            );
          }
          return (
            <div key={template.id} className="flex items-center gap-3 p-3 rounded-xl bg-secondary/10 border border-border/40">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-medium text-foreground">{template.nom_champ}</span>
                  {template.est_obligatoire && <span className="text-[10px] text-red-500">*</span>}
                  <span className="text-[10px] text-muted-foreground bg-secondary/30 px-1.5 py-0.5 rounded">{template.type_champ}</span>
                  <span className="text-[10px] text-muted-foreground bg-primary/10 px-1.5 py-0.5 rounded">template</span>
                </div>
                <input className="w-full mt-1 px-3 py-1.5 text-sm border border-border/40 rounded-lg bg-background/60 focus:border-primary/60 focus:ring-2 focus:ring-primary/15 outline-none transition-all" placeholder={`Valeur pour ${template.nom_champ}`} value={caracteristiques?.[template.nom_champ] || ""} onChange={(e) => handleCaractValueChange(template.nom_champ, e.target.value)} required={template.est_obligatoire} />
              </div>
              {isEditMode && (
                <div className="flex gap-1.5">
                  <button type="button" onClick={() => startEditingTemplate(template)} className="p-1.5 rounded-lg bg-blue-500/10 text-blue-600 hover:bg-blue-500/20 transition-colors" title="Modifier le nom du champ"><Pencil className="h-4 w-4" /></button>
                  {onDeleteTemplate && (
                    <button type="button" onClick={() => onDeleteTemplate(template.id, template.nom_champ)} className="p-1.5 rounded-lg bg-red-500/10 text-red-600 hover:bg-red-500/20 transition-colors" title="Supprimer cette caractéristique"><Trash2 className="h-4 w-4" /></button>
                  )}
                </div>
              )}
            </div>
        ...
        {Object.entries(caracteristiques || {}).filter(([key]) => !templatesDisponibles.some((t) => t.nom_champ === key)).map(([nomChamp, valeur]) => {
          ...
          if (isEditing) {
            return (
              <div key={nomChamp} className="flex items-center gap-3 p-3 rounded-xl bg-primary/10 border border-primary/30">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-1">
                    <input className="px-3 py-1.5 text-sm border border-border/40 rounded-lg bg-background/60 focus:border-primary/60 focus:ring-2 focus:ring-primary/15 outline-none transition-all" placeholder="Nom du champ" value={newCaractNom} onChange={(e) => setNewCaractNom(e.target.value)} />
                    <input className="px-3 py-1.5 text-sm border border-border/40 rounded-lg bg-background/60 focus:border-primary/60 focus:ring-2 focus:ring-primary/15 outline-none transition-all" placeholder="Valeur" value={newCaractValeur} onChange={(e) => setNewCaractValeur(e.target.value)} />
                  </div>
                </div>
                <div className="flex gap-1.5">
                  <button type="button" onClick={saveEditingCaract} disabled={!newCaractNom.trim() || !newCaractValeur.trim()} className="p-1.5 rounded-lg bg-emerald-500/10 text-emerald-600 hover:bg-emerald-500/20 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"><CheckCircle className="h-4 w-4" /></button>
                  <button type="button" onClick={cancelEditing} className="p-1.5 rounded-lg bg-red-500/10 text-red-600 hover:bg-red-500/20 transition-colors"><X className="h-4 w-4" /></button>
                </div>
              </div>
            );
          ...
          return (
            <div key={nomChamp} className="flex items-center gap-3 p-3 rounded-xl bg-primary/5 border border-primary/20">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-medium text-primary">{nomChamp}</span>
                  <span className="text-[10px] text-muted-foreground bg-secondary/30 px-1.5 py-0.5 rounded">personnalisé</span>
                </div>
                <input className="w-full mt-1 px-3 py-1.5 text-sm border border-border/40 rounded-lg bg-background/60 focus:border-primary/60 focus:ring-2 focus:ring-primary/15 outline-none transition-all" placeholder={`Valeur pour ${nomChamp}`} value={valeur} onChange={(e) => handleCaractValueChange(nomChamp, e.target.value)} />
              </div>
              <div className="flex gap-1.5">
                <button type="button" onClick={() => startEditingCaract(nomChamp)} className="p-1.5 rounded-lg bg-blue-500/10 text-blue-600 hover:bg-blue-500/20 transition-colors" title="Modifier le nom"><Pencil className="h-4 w-4" /></button>
                <button type="button" onClick={() => handleRemoveCaracteristique(nomChamp)} className="p-1.5 rounded-lg bg-red-500/10 text-red-600 hover:bg-red-500/20 transition-colors" title="Supprimer"><Trash2 className="h-4 w-4" /></button>
              </div>
            </div>
          );
        })}
        {!showAddCaract ? (
          <button type="button" onClick={() => setShowAddCaract(true)} className="w-full py-2.5 rounded-xl border-2 border-dashed border-border/50 hover:border-primary/40 hover:bg-primary/5 transition-all duration-200 text-sm text-muted-foreground hover:text-primary font-medium flex items-center justify-center gap-2">
            <Plus className="h-4 w-4" /> Ajouter une caractéristique personnalisée
          </button>
        ) : (
          <div className="p-4 rounded-xl border-2 border-primary/30 bg-primary/5 space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                ...
                <input className={INPUT} placeholder="Ex: Couleur, Poids..." value={newCaractNom} onChange={(e) => setNewCaractNom(e.target.value)} />
              </div>
              <div className="space-y-1">
                <label className="text-xs font-medium text-muted-foreground">Valeur *</label>
                <input className={INPUT} placeholder="Ex: Rouge, 1.5kg..." value={newCaractValeur} onChange={(e) => setNewCaractValeur(e.target.value)} />
              </div>
            </div>
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                <label className="text-xs font-medium text-muted-foreground">Type:</label>
                <select className="px-3 py-1.5 text-sm border border-border/60 rounded-lg bg-background/60 outline-none focus:border-primary/60 transition-all" value={newCaractType} onChange={(e) => setNewCaractType(e.target.value as any)}>
              ...
              <label className="flex items-center gap-2 text-xs font-medium text-muted-foreground cursor-pointer">
                <input type="checkbox" checked={newCaractObligatoire} onChange={(e) => setNewCaractObligatoire(e.target.checked)} className="rounded border-border/60 text-primary focus:ring-primary/20" /> Obligatoire
              </label>
            </div>
            <div className="flex gap-2">
              <button type="button" onClick={handleAddCaracteristique} disabled={!newCaractNom.trim() || !newCaractValeur.trim()} className="px-4 py-2 text-sm font-medium rounded-lg bg-primary text-primary-foreground hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed transition-all">Ajouter</button>
              <button type="button" onClick={() => { setShowAddCaract(false); setNewCaractNom(""); setNewCaractValeur(""); setNewCaractType("texte"); setNewCaractObligatoire(false); }} className="px-4 py-2 text-sm font-medium rounded-lg border border-border/60 hover:bg-secondary/50 transition-all">Annuler</button>
            </div>
          </div>
        )}
      </div>
...
const ExistingImagesManager = ({ selectedProduit, existingImages, setExistingImages, setMainImage, deleteImage, handleApiError, toast }: { selectedProduit: Produit | null; existingImages: { id: number; url: string; alt: string; ordre?: number }[]; setExistingImages: React.Dispatch<React.SetStateAction<{ id: number; url: string; alt: string; ordre?: number }[]>>; setMainImage: any; deleteImage: any; handleApiError: (error: any, fallback: string) => void; toast: any }) => {
  ...
  return (
    <div className="space-y-2">
      <h4 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-1.5"><ImageIcon className="h-3.5 w-3.5" /> Images existantes</h4>
      {existingImages.length === 0 ? (
        <div className="flex items-center gap-2 p-3 rounded-xl bg-secondary/20 border border-border/40 text-xs text-muted-foreground"><AlertCircle className="h-4 w-4 shrink-0" /> Aucune image enregistrée.</div>
      ) : (
            ...
            <div key={img.id} className="border border-border/40 rounded-xl overflow-hidden bg-secondary/10 hover:bg-secondary/20 transition-all">
              ...
              <div className="p-2 space-y-1.5">
                ...
                <div className="flex gap-1">
                  <button type="button" onClick={async () => { try { await setMainImage.mutateAsync({ produitId: selectedProduit.id, imageId: img.id }); const details = await api.get(`/produits/${selectedProduit.id}`); setExistingImages(details?.data?.data?.images ?? []); toast({ title: "Image principale mise à jour" }); } catch (e: any) { handleApiError(e, "Impossible de définir l'image principale."); } }} className="flex-1 py-1 text-[10px] font-medium rounded-lg border border-primary/20 bg-primary/5 text-primary hover:bg-primary/10 transition-colors">Principale</button>
                  <button type="button" onClick={async () => { try { await deleteImage.mutateAsync(img.id); setExistingImages((prev) => prev.filter((p) => p.id !== img.id)); toast({ title: "Image supprimée" }); } catch (e: any) { handleApiError(e, "Impossible de supprimer l'image."); } }} className="flex-1 py-1 text-[10px] font-medium rounded-lg border border-destructive/20 bg-destructive/5 text-destructive hover:bg-destructive/10 transition-colors">Supprimer</button>
                </div>
              </div>
            </div>
          ))}
...
const ProductMobileCard = ({ product, onEdit, onDelete, onView, expanded, onToggleExpand }: any) => {
  ...
  return (
    <div className="bg-card border border-border/50 rounded-xl p-4 space-y-3 hover:border-primary/30 transition-all duration-200 hover:shadow-md">
      <div className="flex gap-3">
        <div className="shrink-0">
          ...
          {product.images && product.images.length > 0 ? (
            ...
            <div className="h-16 w-16 rounded-lg border border-border/50 flex items-center justify-center bg-secondary/20"><Package className="h-5 w-5 text-muted-foreground/30" /></div>
          )}
        </div>
        <div className="flex-1 min-w-0">
          ...
          <div className="text-xs mt-1 text-muted-foreground/70 truncate">{product.categorie?.nom ?? "-"}{product.sous_categorie?.nom ? ` › ${product.sous_categorie.nom}` : ""}</div>
        </div>
        <div className="text-right shrink-0">
          ...
          <span className={`inline-block px-2 py-0.5 rounded-full text-[10px] font-medium mt-1 ${product.est_dispo ? "bg-emerald-500/10 text-emerald-600 border border-emerald-500/20" : "bg-red-500/10 text-red-600 border border-red-500/20"}`}>
            {product.est_dispo ? "✓ Disponible" : "✕ Indisponible"}
          </span>
        </div>
      </div>
      {Object.keys(caracteristiques).length > 0 && (
        <button onClick={onToggleExpand} className="flex items-center gap-2 text-xs text-muted-foreground hover:text-primary transition-colors">
      ...
      <div className="flex justify-end gap-2 pt-2 border-t border-border/50">
        <button onClick={() => onView(product)} className="p-2 rounded-lg bg-white border border-border/50 hover:bg-secondary/50 transition-colors shadow-sm" title="Voir détails"><Eye className="h-4 w-4 text-foreground" /></button>
        <button onClick={() => onEdit(product)} className="p-2 rounded-lg bg-white border border-border/50 hover:bg-secondary/50 transition-colors shadow-sm" title="Modifier"><Pencil className="h-4 w-4 text-foreground" /></button>
        <button onClick={() => onDelete(product)} className="p-2 rounded-lg bg-white border border-border/50 hover:bg-destructive/10 transition-colors shadow-sm text-destructive" title="Supprimer"><Trash2 className="h-4 w-4" /></button>
      </div>
    </div>
  );
};

const ModalHeader = ({ icon: Icon, title, onClose, accent = false }: { icon: any; title: string; onClose: () => void; accent?: boolean }) => (
  <div className={`flex items-center justify-between px-6 py-4 border-b border-border/50 rounded-t-2xl ${accent ? "bg-gradient-to-r from-primary/5 to-transparent" : ""}`}>
    <div className="flex items-center gap-3">
      <div className="p-2 rounded-xl bg-primary/10 text-primary"><Icon className="h-4 w-4" /></div>
      <h2 className="text-base font-bold text-foreground">{title}</h2>
    </div>
    <button onClick={onClose} className="p-1.5 rounded-lg text-muted-foreground hover:text-foreground hover:bg-secondary/60 transition-all duration-200"><X className="h-4 w-4" /></button>
  </div>
);

const ModalFooter = ({ onCancel, onConfirm, confirmLabel = "Enregistrer", confirmIcon: ConfirmIcon = Save, danger = false, isLoading = false }: { onCancel: () => void; onConfirm: () => void; confirmLabel?: string; confirmIcon?: any; danger?: boolean; isLoading?: boolean }) => (
  <div className="flex justify-end gap-2.5 px-6 py-4 border-t border-border/50 bg-secondary/10 rounded-b-2xl">
    <button onClick={onCancel} disabled={isLoading} className="px-4 py-2 text-sm border border-border/60 rounded-xl hover:bg-secondary/50 text-muted-foreground hover:text-foreground transition-all duration-200 disabled:opacity-50">Annuler</button>
    <button onClick={onConfirm} disabled={isLoading} className={`px-5 py-2 text-sm font-semibold rounded-xl inline-flex items-center gap-2 transition-all duration-200 shadow-sm ${danger ? "bg-destructive text-destructive-foreground hover:bg-destructive/90" : "bg-primary text-primary-foreground hover:bg-primary/90 hover:shadow-md hover:shadow-primary/20"} disabled:opacity-50 disabled:cursor-not-allowed`}>
...
const AdminProduits = () => {
  ...
  const apiFilters = useMemo<ProductFilters>(() => ({ search: searchTerm || undefined, est_dispo: dispoFilter === "all" ? undefined : dispoFilter === "available" ? 1 : 0 }), [searchTerm, dispoFilter]);
        ...
        const produitsAvecCaracts = await Promise.all(
          normalizedProduits.map(async (produit) => {
            try {
              ...
              caracts.forEach((c: any) => { caractsObj[c.nom_champ] = c.valeur; });
              return { ...produit, caracteristiques: caractsObj };
            } catch (error) {
              console.error(`Erreur chargement caractéristiques pour produit ${produit.id}:`, error);
              return { ...produit, caracteristiques: {} };
            }
          })
        );
  ...
  const filteredProduits = useMemo(() => {
    ...
    if (filterSousCategorieId) result = result.filter(p => String(p.id_sous_categorie) === filterSousCategorieId);
  ...
  const loadTemplatesForSousCategorie = async (sousCategorieId: string) => {
    if (!sousCategorieId) { setCreateTemplates([]); setCreateCaracteristiques({}); return; }
    try {
      ...
      templates.forEach((t: TemplateCaracteristique) => { if (t.valeur_par_defaut) { initialCaracts[t.nom_champ] = t.valeur_par_defaut; } });
  ...
  const loadTemplatesAndValues = async (sousCategorieId: string) => {
    try {
      ...
      for (const template of templates) {
        ...
        } catch (error) { console.error(`Erreur chargement valeurs pour ${template.nom_champ}:`, error); valeursMap[template.nom_champ] = []; }
      }
  ...
  const deleteCaracteristique = async () => {
    ...
    try {
      ...
      setEditCaracteristiques(prev => { const newCaracts = { ...prev }; delete newCaracts[caractToDelete.nom_champ]; return newCaracts; });
      toast({ title: "Caractéristique supprimée", description: `"${caractToDelete.nom_champ}" a été supprimé avec succès` });
      setShowDeleteCaractModal(false);
      setCaractToDelete(null);
    } catch (error: any) {
      console.error("Erreur lors de la suppression:", error);
      toast({ title: "Erreur", description: error?.response?.data?.message || "Impossible de supprimer cette caractéristique", variant: "destructive" });
    } finally { setIsDeletingCaract(false); }
  };

  const editTemplate = async (templateId: number, oldNomChamp: string, newNomChamp: string) => {
    try {
      await api.put(`/templates-caracteristiques/${templateId}`, { nom_champ: newNomChamp });
      setEditTemplates(prev => prev.map(t => t.id === templateId ? { ...t, nom_champ: newNomChamp } : t));
      setEditCaracteristiques(prev => { const newCaracts = { ...prev }; const value = newCaracts[oldNomChamp] || ""; delete newCaracts[oldNomChamp]; newCaracts[newNomChamp] = value; return newCaracts; });
      toast({ title: "Caractéristique modifiée", description: `"${oldNomChamp}" renommé en "${newNomChamp}"` });
    } catch (error: any) {
      console.error("Erreur lors de la modification:", error);
      toast({ title: "Erreur", description: error?.response?.data?.message || "Impossible de modifier cette caractéristique", variant: "destructive" });
      throw error;
    }
  };
  ...
  const generateReference = async (prefixKey: string) => {
    ...
    } catch { toast({ title: "Erreur", description: "Impossible de générer la référence", variant: "destructive" }); }
  };
  ...
  const saveCaracteristiques = async (produitId: number, caracteristiques: Record<string, string>) => {
    ...
    try {
      await api.post(`/produits/${produitId}/caracteristiques/sync`, { caracteristiques: entries.map(([nom, valeur]) => ({ nom_champ: nom, valeur: valeur.trim(), type_champ: "texte", est_obligatoire: false })) });
    } catch (error) { console.error("Erreur lors de la sauvegarde des caractéristiques:", error); throw error; }
  };
  ...
  const handleOpenEdit = async (p: Produit) => {
    ...
    } catch (error) { console.error("Erreur chargement caractéristiques:", error); }
    try {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setShowDeleteAlert(false); setSelectedProduit(null);
      await refetch();
  ...
  const handleCategorySubmit = async () => {
    const payload = { nom: categoryForm.nom, type: categoryForm.type, parent_id: categoryForm.parent_id ? Number(categoryForm.parent_id) : null, ordre_tri: Number(categoryForm.ordre_tri || 0) };
    try {
      if (selectedCategory?.id) await api.put(`/categories/${selectedCategory.id}`, payload);
      else await api.post("/categories", payload);
      setShowCategoryModal(false); setSelectedCategory(null); setCategoryForm(initialCategoryForm);
  ...
  const handleCaracteristiqueFilterChange = (nomChamp: string, valeur: string, checked: boolean) => {
    setFilterCaracteristiques(prev => {
      ...
      if (checked) { return { ...prev, [nomChamp]: [...current, valeur] }; } else { return { ...prev, [nomChamp]: current.filter(v => v !== valeur) }; }
    });
  };
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground mb-1"><Box className="h-3.5 w-3.5" /><span>Catalogue</span><ChevronDown className="h-3 w-3 rotate-[-90deg]" /><span className="text-foreground font-medium">Produits</span></div>
        ...
        <button onClick={() => setShowModal(true)} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-primary text-primary-foreground font-semibold text-sm hover:bg-primary/90 hover:shadow-lg hover:shadow-primary/25 transition-all duration-200 w-full sm:w-auto justify-center">
          <PlusCircle className="h-4 w-4" /> Ajouter un produit
        </button>
      </div>

      <div className="flex gap-1.5 flex-wrap border-b border-border/50 pb-0">
        {[{ id: "produits", label: "Produits", icon: Package }, { id: "categories", label: "Catégories", icon: FolderTree }, { id: "sous-categories", label: "Sous-catégories", icon: Layers }].map((tab) => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id as typeof activeTab)} className={`inline-flex items-center gap-2 px-4 py-2.5 rounded-t-xl text-sm font-medium transition-all duration-200 border-b-2 ${activeTab === tab.id ? "border-primary text-primary bg-primary/5" : "border-transparent text-muted-foreground hover:text-foreground hover:bg-secondary/30"}`}>
        ...
        <div className="space-y-4">
          <div className="flex flex-col gap-3">
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="relative flex-1">
                ...
                <input className="w-full pl-9 pr-3 py-2.5 border border-border/60 rounded-xl text-sm bg-background/60 focus:border-primary/60 focus:ring-2 focus:ring-primary/15 outline-none transition-all" placeholder="Rechercher un produit..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
              </div>
              <div className="flex gap-2">
                <select className="px-3 py-2.5 border border-border/60 rounded-xl bg-background/60 text-sm outline-none focus:border-primary/60 transition-all" value={dispoFilter} onChange={(e) => setDispoFilter(e.target.value as any)}>
                  <option value="all">Tous</option>
                  <option value="available">Disponibles</option>
                  <option value="unavailable">Indisponibles</option>
                </select>
                <button onClick={() => setShowFilters(!showFilters)} className={`px-4 py-2.5 rounded-xl border border-border/60 text-sm font-medium transition-all flex items-center gap-2 ${showFilters ? "bg-primary/10 text-primary border-primary/30" : "hover:bg-secondary/50"}`}>
                  <Filter className="h-4 w-4" /> Filtres
                </button>
                <div className="flex gap-1 border border-border/60 rounded-xl p-1 bg-background/60">
                  <button onClick={() => setViewMode("table")} className={`p-1.5 rounded-lg transition-all ${viewMode === "table" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}><List className="h-4 w-4" /></button>
                  <button onClick={() => setViewMode("grid")} className={`p-1.5 rounded-lg transition-all ${viewMode === "grid" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}><Grid className="h-4 w-4" /></button>
                </div>
              </div>
            </div>
            {showFilters && (
              <div className="border border-border/50 rounded-2xl bg-card p-4 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <select className={INPUT} value={filterCategorieId} onChange={(e) => { setFilterCategorieId(e.target.value); setFilterSousCategorieId(""); setFilterCaracteristiques({}); }}>
                      <option value="">Toutes les catégories</option>
                      {normalizedCategories.map((c: any) => (<option key={c.id} value={c.id}>{c.nom}</option>))}
                    </select>
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Sous-catégorie</label>
                    <select className={INPUT} value={filterSousCategorieId} onChange={(e) => { setFilterSousCategorieId(e.target.value); setFilterCaracteristiques({}); }} disabled={!filterCategorieId}>
                      <option value="">Toutes les sous-catégories</option>
                      {normalizedSousCategories.filter((sc: any) => String(sc.id_categorie) === filterCategorieId).map((sc: any) => (<option key={sc.id} value={sc.id}>{sc.nom}</option>))}
                    </select>
                  </div>
                </div>
                {filterSousCategorieId && templatesBySousCategorie.length > 0 && (
                  <div className="border-t border-border/50 pt-4">
                    <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3 block">Caractéristiques</label>
                          ...
                          <div className="space-y-1 max-h-32 overflow-y-auto">
                            {(valeursByTemplate[template.nom_champ] || []).map((valeur) => (
                              <label key={valeur} className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors cursor-pointer">
                                <input type="checkbox" checked={(filterCaracteristiques[template.nom_champ] || []).includes(valeur)} onChange={(e) => handleCaracteristiqueFilterChange(template.nom_champ, valeur, e.target.checked)} className="rounded border-border/60 text-primary focus:ring-primary/20" />
                                {valeur}
                              </label>
                            ))}
                          </div>
                ...
                <div className="flex justify-end pt-2 border-t border-border/50">
                  <button onClick={() => { setFilterCategorieId(""); setFilterSousCategorieId(""); setFilterCaracteristiques({}); setTemplatesBySousCategorie([]); setValeursByTemplate({}); }} className="px-4 py-2 text-sm font-medium rounded-lg border border-border/60 hover:bg-secondary/50 transition-all">Réinitialiser les filtres</button>
                </div>
              </div>
            )}
          </div>
                        ...
                        <tr key={p.id} className="border-b border-border/30 hover:bg-secondary/10 transition-colors last:border-0">
                          <td className="p-4">
                            ...
                            {p.images && p.images.length > 0 ? (
                              ...
                              <div className="h-11 w-11 rounded-lg border border-border/40 flex items-center justify-center bg-secondary/20"><Package className="h-4 w-4 text-muted-foreground/30" /></div>
                            )}
                          </td>
                          ...
                          <td className="p-4 text-xs text-muted-foreground">{p.categorie?.nom ?? "-"}{p.sous_categorie?.nom ? <span className="text-muted-foreground/60"> › {p.sous_categorie.nom}</span> : ""}</td>
                          ...
                          <td className="p-4">
                            <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${p.est_dispo ? "bg-emerald-500/10 text-emerald-600 border border-emerald-500/20" : "bg-red-500/10 text-red-600 border border-red-500/20"}`}>
                              {p.est_dispo ? <CheckCircle className="h-3 w-3" /> : <AlertCircle className="h-3 w-3" />}
                              {p.est_dispo ? "Disponible" : "Indisponible"}
                            </span>
                          </td>
                          <td className="p-4">
                            {Object.keys(p.caracteristiques || {}).length > 0 && (
                              <button onClick={() => toggleExpand(p.id)} className="flex items-center gap-1 text-xs text-muted-foreground hover:text-primary transition-colors">
                                {isExpanded ? <ChevronDown className="h-3.5 w-3.5" /> : <ChevronRight className="h-3.5 w-3.5" />}
                                {isExpanded ? "Masquer" : "Voir"} ({Object.keys(p.caracteristiques || {}).length})
                              </button>
                            )}
                            {isExpanded && (
                          ...
                          <td className="p-4">
                            <div className="flex justify-end gap-1.5">
                              <button onClick={() => navigate(`/DashboardAdmin/produits/${p.id}`)} className="p-1.5 rounded-lg bg-white border border-border/40 hover:bg-secondary/50 transition-colors shadow-sm" title="Voir"><Eye className="h-3.5 w-3.5 text-foreground" /></button>
                              <button onClick={() => handleOpenEdit(p)} className="p-1.5 rounded-lg bg-white border border-border/40 hover:bg-secondary/50 transition-colors shadow-sm" title="Modifier"><Pencil className="h-3.5 w-3.5 text-foreground" /></button>
                              <button onClick={() => { setSelectedProduit(p); setShowDeleteAlert(true); }} className="p-1.5 rounded-lg bg-white border border-border/40 hover:bg-destructive/10 transition-colors shadow-sm text-destructive" title="Supprimer"><Trash2 className="h-3.5 w-3.5" /></button>
                            </div>
                          </td>
                        </tr>
                      );
                  ...
                  <div key={p.id} className="bg-card border border-border/50 rounded-2xl overflow-hidden hover:border-primary/30 transition-all duration-200 hover:shadow-lg hover:shadow-black/10">
                    <div className="relative">
                      ...
                      {p.images && p.images.length > 0 ? (
                        ...
                        <div className="w-full h-40 flex items-center justify-center bg-secondary/20"><Package className="h-12 w-12 text-muted-foreground/20" /></div>
                      )}
                      <span className={`absolute top-2.5 right-2.5 px-2 py-0.5 rounded-full text-[10px] font-semibold ${p.est_dispo ? "bg-emerald-500 text-white" : "bg-red-500 text-white"}`}>
                        {p.est_dispo ? "Disponible" : "Indisponible"}
                      </span>
                    </div>
                    <div className="p-4 space-y-3">
                      ...
                      {Object.keys(p.caracteristiques || {}).length > 0 && (
                        <div className="space-y-1">
                          <button onClick={() => toggleExpand(p.id)} className="flex items-center gap-1 text-xs text-muted-foreground hover:text-primary transition-colors">
                            ...
                            <span className="text-[10px] text-muted-foreground/50">({Object.keys(p.caracteristiques || {}).length})</span>
                          </button>
                          {isExpanded && (
                      ...
                      <div className="flex gap-1.5">
                        <button onClick={() => navigate(`/DashboardAdmin/produits/${p.id}`)} className="flex-1 p-1.5 rounded-lg bg-white border border-border/40 hover:bg-secondary/50 transition-colors shadow-sm flex items-center justify-center"><Eye className="h-3.5 w-3.5 text-foreground" /></button>
                        <button onClick={() => handleOpenEdit(p)} className="flex-1 p-1.5 rounded-lg bg-white border border-border/40 hover:bg-secondary/50 transition-colors shadow-sm flex items-center justify-center"><Pencil className="h-3.5 w-3.5 text-foreground" /></button>
                        <button onClick={() => { setSelectedProduit(p); setShowDeleteAlert(true); }} className="flex-1 p-1.5 rounded-lg bg-white border border-border/40 hover:bg-destructive/10 transition-colors shadow-sm flex items-center justify-center text-destructive"><Trash2 className="h-3.5 w-3.5" /></button>
                      </div>
                    </div>
                  </div>
                );
          ...
          <div className="md:hidden space-y-3">
            {filteredProduits.length === 0 ? (
              ...
              <ProductMobileCard key={p.id} product={p} onEdit={handleOpenEdit} onDelete={(product: Produit) => { setSelectedProduit(product); setShowDeleteAlert(true); }} onView={(product: Produit) => navigate(`/DashboardAdmin/produits/${product.id}`)} expanded={expandedItems[p.id] || false} onToggleExpand={() => toggleExpand(p.id)} />
            ))}
          </div>
        </div>
      )}
      ...
      {activeTab === "categories" && (
        <div className="space-y-4">
          <button onClick={() => { setSelectedCategory(null); setCategoryForm(initialCategoryForm); setShowCategoryModal(true); }} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-primary text-primary-foreground font-semibold text-sm hover:bg-primary/90 transition-all duration-200">
              ...
              <tbody>
                {normalizedCategories.map((c: any) => (
                  <tr key={c.id} className="border-b border-border/30 hover:bg-secondary/10 transition-colors last:border-0">
                    ...
                    <td className="p-4"><span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-secondary/50 text-muted-foreground border border-border/40">{c.type}</span></td>
                    <td className="p-4">
                      <div className="flex justify-end gap-1">
                        <button onClick={() => { setSelectedCategory(c); setCategoryForm({ nom: c.nom, type: c.type, parent_id: c.parent_id ? String(c.parent_id) : "", ordre_tri: String(c.ordre_tri ?? 0) }); setShowCategoryModal(true); }} className="p-1.5 rounded-lg border border-border/40 hover:bg-secondary/50 transition-colors"><Pencil className="h-3.5 w-3.5" /></button>
                        <button onClick={async () => { await api.delete(`/categories/${c.id}`); await refetchCategories(); }} className="p-1.5 rounded-lg border border-border/40 hover:bg-destructive/10 transition-colors text-destructive"><Trash2 className="h-3.5 w-3.5" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      {activeTab === "sous-categories" && (
        <div className="space-y-4">
          <button onClick={() => { setSelectedSousCategory(null); setSousCategoryForm({ id_categorie: "", nom: "" }); setShowSousCategoryModal(true); }} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-primary text-primary-foreground font-semibold text-sm hover:bg-primary/90 transition-all duration-200">
              ...
              <tbody>
                {normalizedSousCategories.map((sc: any) => (
                  <tr key={sc.id} className="border-b border-border/30 hover:bg-secondary/10 transition-colors last:border-0">
                    ...
                    <td className="p-4"><span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-secondary/50 text-muted-foreground border border-border/40">{sc.categorie?.nom || sc.id_categorie}</span></td>
                    <td className="p-4">
                      <div className="flex justify-end gap-1">
      ...
      {showEditModal && (
        <ModalPortal onClose={closeEditModal}>
          <div className={`${MODAL_PANEL} max-w-2xl max-h-[88vh]`}>
            ...
            <ModalFooter onCancel={closeEditModal} onConfirm={handleEdit} confirmLabel="Enregistrer" isLoading={isUpdating} />
          </div>
        </ModalPortal>
      )}
      ...
      {showCategoryModal && (
        <ModalPortal onClose={() => setShowCategoryModal(false)}>
          <div className={`${MODAL_PANEL} max-w-md`}>
            <ModalHeader icon={FolderTree} title={selectedCategory ? "Modifier la catégorie" : "Nouvelle catégorie"} onClose={() => setShowCategoryModal(false)} />
      ...
      {showSousCategoryModal && (
        <ModalPortal onClose={() => setShowSousCategoryModal(false)}>
          <div className={`${MODAL_PANEL} max-w-md`}>
            <ModalHeader icon={Layers} title={selectedSousCategory ? "Modifier la sous-catégorie" : "Nouvelle sous-catégorie"} onClose={() => setShowSousCategoryModal(false)} />
            <div className="px-6 py-5 space-y-4">
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Nom</label>
            ...
            <ModalFooter onCancel={() => setShowSousCategoryModal(false)} onConfirm={async () => { if (!sousCategoryForm.nom || !sousCategoryForm.id_categorie) { toast({ title: "Erreur", description: "Veuillez remplir tous les champs", variant: "destructive" }); return; } try { if (selectedSousCategory) await updateSousCategoryMutation.mutateAsync({ id: selectedSousCategory.id, updatedData: sousCategoryForm }); else await createSousCategoryMutation.mutateAsync(sousCategoryForm); setShowSousCategoryModal(false); setSelectedSousCategory(null); setSousCategoryForm({ id_categorie: "", nom: "" }); await refetchSousCategories(); toast({ title: "Sous-catégorie enregistrée" }); } catch (e: any) { handleApiError(e, "Impossible d'enregistrer la sous-catégorie."); } }} />
          </div>
        </ModalPortal>
      )}
          ...
          <div className={`${MODAL_PANEL} max-w-md`}>
            <div className="px-6 py-6 space-y-4">
              <div className="flex items-start gap-4">
                <div className="p-3 rounded-xl bg-destructive/10 text-destructive shrink-0"><Trash2 className="h-5 w-5" /></div>
                <div>
                  <h3 className="font-bold text-base text-foreground">Supprimer le produit</h3>
                  <p className="text-sm text-muted-foreground mt-1 leading-relaxed">Confirmer la suppression de <span className="font-semibold text-foreground">« {selectedProduit?.nom} »</span> ? Cette action est irréversible.</p>
                </div>
              </div>
            </div>
            <ModalFooter onCancel={() => setShowDeleteAlert(false)} onConfirm={handleDelete} confirmLabel="Supprimer définitivement" confirmIcon={Trash2} danger />
          </div>
        </ModalPortal>
      )}
      ...
      {showDeleteCaractModal && (
        <ModalPortal onClose={() => setShowDeleteCaractModal(false)}>
          <div className={`${MODAL_PANEL} max-w-md`}>
            <div className="px-6 py-6 space-y-4">
              <div className="flex items-start gap-4">
                <div className="p-3 rounded-xl bg-destructive/10 text-destructive shrink-0"><AlertCircle className="h-5 w-5" /></div>
                <div>
                  <h3 className="font-bold text-base text-foreground">Supprimer la caractéristique</h3>
                  <p className="text-sm text-muted-foreground mt-1 leading-relaxed">Êtes-vous sûr de vouloir supprimer la caractéristique <span className="font-semibold text-foreground">« {caractToDelete?.nom_champ} »</span> ?</p>
                  <p className="text-xs text-muted-foreground mt-2 text-red-500">⚠️ Cette action supprimera également toutes les valeurs associées à cette caractéristique.</p>
                </div>
              </div>
            </div>
            <div className="flex justify-end gap-2.5 px-6 py-4 border-t border-border/50 bg-secondary/10 rounded-b-2xl">
              <button onClick={() => setShowDeleteCaractModal(false)} disabled={isDeletingCaract} className="px-4 py-2 text-sm border border-border/60 rounded-xl hover:bg-secondary/50 text-muted-foreground hover:text-foreground transition-all duration-200 disabled:opacity-50">Annuler</button>
              <button onClick={deleteCaracteristique} disabled={isDeletingCaract} className="px-5 py-2 text-sm font-semibold rounded-xl inline-flex items-center gap-2 transition-all duration-200 shadow-sm bg-destructive text-destructive-foreground hover:bg-destructive/90 disabled:opacity-50 disabled:cursor-not-allowed">
