...
const DashboardFavoris = () => {
  const { removeFavoriteLocal } = useShop();
  ...
  const remove = async (produit: Produit) => {
    try {
      ...
      removeFavoriteLocal(produit.id);
    ...
    <section className="min-h-[calc(100vh-12rem)] bg-black px-5 py-6 text-white sm:px-7 md:px-9 md:py-8">
      <div className="mx-auto max-w-7xl">
        <header className="mb-8">
          ...
          <p className="mt-1 inline-block border-b border-white/60 pb-1 text-sm font-medium italic text-white/75 sm:text-base">Consulte tes produits favoris</p>
        </header>
        {produits.length > 0 && 
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {produits.map((produit) => 
          <article key={produit.id} className="group relative overflow-hidden rounded-xl bg-white p-3 text-black shadow-sm">
            <div className="flex items-center justify-between text-[10px] italic text-black/70">
              <span>{produit.type_produit || "Produit"}</span>
              <button aria-label="Retirer des favoris" onClick={() => remove(produit)} className="rounded p-1 transition hover:bg-black hover:text-white">
                <Trash2 className="h-3.5 w-3.5" />
              </button>
            </div>
            <Link to={`/produit/${produit.id}`} className="mt-2 block h-36 sm:h-40">
              <ProductImage produit={produit} className="h-full w-full object-contain" />
            </Link>
            <Link to={`/produit/${produit.id}`} className="mt-3 block text-xs font-bold leading-snug transition hover:underline">{produit.nom}</Link>
            <p className="mt-4 text-center text-lg font-bold">{formatPrice(produit.prix, produit.devise)}</p>
          </article>)}
        </div>}
        <div className="mt-8 rounded-xl bg-white px-6 py-9 text-center text-black shadow-sm sm:py-10">
          <p className="text-sm italic text-black/65 sm:text-base">Besoin d&apos;ajouter des produits au favoris ?</p>
          <div className="mt-3 flex flex-wrap items-center justify-center gap-x-2 gap-y-1 text-sm font-semibold sm:text-base">
            <Link to="/catalogue" className="transition hover:text-primary hover:underline">Ajoute-les ici</Link>
            <span className="font-normal text-black/65">ou</span>
            <Link to="/configurateur" className="transition hover:text-primary hover:underline">Configure ton propre setup</Link>
          </div>
        </div>
