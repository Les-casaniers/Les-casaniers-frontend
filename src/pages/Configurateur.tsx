...
//               <div>
//                 <div className="font-display font-bold">Votre buildd</div>
...
const stepsConfig = [
  { key: "case", title: "La Forteresse — Boîtier", mascot: "La Forteresse", icon: Box, hint: "Choisissez un boîtier adapté à votre carte mère" },
  { key: "cpu", title: "Le Cerveau — Processeur", mascot: "Le Cerveau", icon: Cpu, hint: "Le cœur de votre configuration" },
  { key: "motherboard", title: "L'Architecte — Carte mère", mascot: "L'Architecte", icon: CircuitBoard, hint: "Assurez-vous de la compatibilité avec votre CPU" },
  { key: "cooling", title: "Sub-Zero — Refroidissement", mascot: "Sub-Zero", icon: Snowflake, hint: "Gardez votre CPU au frais" },
  { key: "ram", title: "L'Archiviste — Mémoire RAM", mascot: "L'Archiviste", icon: MemoryStick, hint: "Plus de RAM = plus de multitâche" },
  { key: "storage", title: "L'Éclair — Stockage", mascot: "L'Éclair", icon: HardDrive, hint: "SSD pour la vitesse, HDD pour la capacité" },
  { key: "gpu", title: "Le Titan — Carte graphique", mascot: "Le Titan", icon: MonitorCog, hint: "Essentielle pour le gaming et le montage" },
  { key: "psu", title: "Le Générateur — Alimentation", mascot: "Le Générateur", icon: Zap, hint: "Prévoyez 20% de marge" }
];

type Selections = Record<string, Product | null>;

// Fonction pour filtrer les produits par référence
const filterProductsByReference = (products: Product[], refPrefixes: string[]): Product[] => {
...
const Configurateur = () => {
  ...
  const { data: allProducts = [], isLoading, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (response.data && response.data.data && Array.isArray(response.data.data)) {
        ...
        console.log("❌ Structure inconnue, impossible d'extraire les produits");
        return [];
      }
      ...
      products.forEach((p: Product) => {
        console.log(`  - ID:${p.id} | ${p.nom} | Ref:"${p.reference}" | Stock:${p.quantite_stock}`);
      });
  ...
  const categorizedProducts = useMemo(() => {
    ...
    for (const step of stepsConfig) {
      ...
      console.log(`   Recherche des références commençant par: ${refPrefixes.join(', ')}`);
  ...
  const handleAddToCart = async () => {
    ...
    try {
      const selectedComponents = stepsConfig.map(step => {
  ...
  if (isLoading) {
    return (
      <SiteLayout>
        <MiniHero title="Composez votre machine idéale." description="Chargement des composants..." bg="5.png" />
        <section className="container-x py-20 text-center">
          <div className="animate-pulse">
            <Cpu className="h-16 w-16 mx-auto text-muted-foreground mb-4 animate-spin" />
            <p className="text-muted-foreground">Chargement des composants disponibles...</p>
          </div>
        </section>
      </SiteLayout>
    );
  ...
  return (
    <SiteLayout>
      <MiniHero title="Composez votre machine idéale." description="Choisissez vos composants pièce par pièce" bg="5.png" />
      ...
      <section className="container-x py-10 grid lg:grid-cols-12 gap-8">
        <div className="lg:col-span-8 space-y-6">
          ...
          <div className="card-soft p-5">
            ...
            <div className="flex flex-wrap gap-2 mt-4">
              {stepsConfig.map((step, idx) => (
                ...
                >
                  {selections[step.key] && <Check className="h-3 w-3 inline mr-1" />}
                  {step.mascot}
                </button>
              ))}
            </div>
          ...
          <div className="card-soft p-6 lg:p-8 animate-fade-up">
            <div className="flex items-start gap-4 mb-6">
              ...
              <div>
                <div className="text-xs font-mono uppercase tracking-wider text-accent">{currentStepData.mascot}</div>
                <h2 className="font-display text-2xl lg:text-3xl font-bold">{currentStepData.title}</h2>
                <p className="text-sm text-muted-foreground mt-2 italic">"{currentStepData.hint}"</p>
              </div>
            </div>
            ...
            {currentProducts.length === 0 ? (
              <div className="text-center py-12 bg-secondary/30 rounded-xl">
                ...
                <p className="text-muted-foreground">Aucune donnée pour le moment</p>
                <p className="text-xs text-muted-foreground mt-1">
                  Aucun composant trouvé avec les références {referenceMapping[currentStepData.key as keyof typeof referenceMapping]?.join(', ')}
                </p>
              </div>
            ) : (
              <div className="grid sm:grid-cols-2 gap-3">
            ...
            <div className="flex justify-between items-center mt-6">
              ...
              <Button variant="soft" size="sm" onClick={handleNext} disabled={currentStep === stepsConfig.length - 1}>
        ...
        <aside className="lg:col-span-4 lg:sticky lg:top-32 self-start space-y-4">
          <div className="card-soft p-6 bg-gradient-to-br from-card to-secondary/30">
            <div className="flex items-center gap-3 mb-4">
              <img src={fosa} alt="Le Fosa" className="h-10 w-10 animate-float" />
              <div>
                <div className="font-display font-bold">Votre configuration</div>
            ...
            <div className="space-y-3 text-sm max-h-96 overflow-y-auto">
              {stepsConfig.map((step) => {
                ...
                return (
                  <div key={step.key} className="flex items-start justify-between gap-2 py-2 border-b border-border last:border-0">
                    <div className="min-w-0 flex-1">
                      <div className="text-[10px] uppercase tracking-wider text-muted-foreground">{step.mascot}</div>
                      <div className="font-medium text-sm truncate">
                        {selectedProduct?.nom || <span className="text-muted-foreground italic">Non sélectionné</span>}
                      </div>
                    </div>
            ...
            <div className="mt-4 pt-4 border-t border-border">
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-[10px] uppercase tracking-wider text-muted-foreground">Total estimé</div>
                  <div className="font-display font-bold text-2xl text-primary">{formatAr(total)}</div>
                </div>
                <Button variant="hero" size="lg" onClick={handleAddToCart} disabled={!allDone}>
                  Ajouter au panier <ChevronRight />
                </Button>
              </div>
              {!allDone && (
                <p className="text-xs text-muted-foreground mt-3 text-center">
                  {stepsConfig.length - completedCount} composant(s) à sélectionner
                </p>
              )}
            </div>
          </div>
