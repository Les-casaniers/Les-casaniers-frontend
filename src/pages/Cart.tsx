...
import {
  ...
  ShieldCheck, Truck, Loader2, Heart, X, Check,
  MapPin, Home, Building, Package, ChevronRight,
  Clock, Phone, BadgeCheck
} from "lucide-react";
...
const Row = ({ label, value, accent }: { label: string; value: string; accent?: boolean }) => (
  <div className="flex items-center justify-between">
    ...
    <span className={`tabular-nums text-sm font-medium ${accent ? "text-green-500" : "text-foreground"}`}>{value}</span>
  </div>
);
...
const OrderConfirmationScreen = ({ commandeUuid, onClose }: { commandeUuid: string; onClose: () => void }) => (
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md p-4">
    <div className="bg-background border border-border rounded-2xl shadow-2xl w-full max-w-md text-center p-8 animate-slide-up">
      ...
      <h2 className="font-display text-2xl font-bold text-foreground mb-1">Commande enregistrée !</h2>
      <p className="text-xs font-mono text-muted-foreground mb-6">Réf. {commandeUuid}</p>
      ...
      <div className="bg-secondary/40 border border-border rounded-xl p-4 mb-6 text-left space-y-3">
        <div className="flex items-start gap-3">
          ...
          <p className="text-sm text-foreground leading-relaxed">
            Un responsable vous recontactera sous <span className="font-semibold text-primary">24 à 48h</span> pour finaliser et valider votre commande.
          </p>
        </div>
        <div className="flex items-start gap-3">
          <div className="p-1.5 bg-primary/10 rounded-lg shrink-0 mt-0.5">
  ...
  const handleCommander = async () => {
    ...
    try {
      ...
      const stockChecks: StockCheckResult[] = await Promise.all(
        cartDetailed.map(async (item) => {
          try {
            ...
            return { id: item.product.id, nom: item.product.name, quantite_demandee: item.qty, stock_actuel: 0, suffisant: false, error: true };
          }
        })
      );

      const stockInsuffisant = stockChecks.filter(c => !c.suffisant);
      ...
      const stockUpdates: StockUpdateResult[] = await Promise.all(
        cartDetailed.map(async (item) => {
          try {
            ...
            return { id: item.product.id, nom: item.product.name, ancien_stock: product.quantite_stock, nouveau_stock: nouveauStock, success: true };
          } catch (error) {
            return { id: item.product.id, nom: item.product.name, ancien_stock: 0, nouveau_stock: 0, success: false, error };
          }
        })
      );
      ...
      if (failedUpdates.length > 0) {
        await Promise.all(stockUpdates.filter(u => u.success).map(u =>
          api.put(`/produits/${u.id}`, { quantite_stock: u.ancien_stock, est_dispo: u.ancien_stock > 0 }).catch(console.error)
        ));
        toast({ title: "Erreur", description: "Mise à jour des stocks impossible.", variant: "destructive" });
        return;
      }
      ...
      const commandeData: any = {
        ...
        devise: devisForm.devise,
        adresse_expedition_id: devisForm.besoinLivraison && devisForm.adresseId > 0 ? devisForm.adresseId : null,
      ...
      <SiteLayout>
        <section className="container-x py-12">
          <div className="flex flex-col items-center justify-center py-24">
            ...
            <p className="text-muted-foreground text-sm">Chargement du panier…</p>
          </div>
        </section>
      </SiteLayout>
    );
        ...
        <section className="container-x py-12">
          <div className="relative card-soft p-12 text-center max-w-xl mx-auto overflow-hidden">
            <div className="relative inline-block mb-4">
              ...
              {[...Array(10)].map((_, i) => (
                <Heart key={i} className="absolute fill-rose-500 text-rose-500 animate-heart-orbit"
                  style={{
            ...
            <p className="text-muted-foreground mb-6 text-sm">Votre panier est vide. Découvrez nos configurations !</p>
      ...
      <section className="container-x py-12">
        ...
        <div className="mb-10">
          ...
          <p className="text-muted-foreground mt-2 text-sm">
            {cartDetailed.length} article{cartDetailed.length > 1 ? 's' : ''} sélectionné{cartDetailed.length > 1 ? 's' : ''}
          </p>
        </div>
        ...
        <div className="grid lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 space-y-3">
            {cartDetailed.map((item) => {
              ...
              return (
                <div key={item.id} className="card-soft p-4 flex gap-4 hover-lift group">
                  <Link to={`/produit/${item.product.id}`} className="shrink-0">
                    <img
                      src={imageUrl} alt={item.product.name}
                      className="h-24 w-24 rounded-xl object-cover transition-transform group-hover:scale-105"
                      onError={(e) => { (e.target as HTMLImageElement).src = fosa; }}
                    />
                  </Link>
                  <div className="flex-1 min-w-0 flex flex-col justify-between py-0.5">
                    <div>
                      <div className="text-[10px] font-mono uppercase tracking-wider text-accent mb-0.5">{item.product.category}</div>
                      <Link to={`/produit/${item.product.id}`} className="font-display font-bold text-base leading-tight hover:text-accent transition-colors line-clamp-1">
                        {item.product.name}
                      </Link>
                      <p className="text-xs text-muted-foreground italic line-clamp-1 mt-0.5">"{item.product.tagline}"</p>
                    </div>
                    <div className="flex items-center justify-between mt-3">
                      {/* Contrôle quantité */}
                      <div className="flex items-center bg-secondary rounded-full h-8">
                        <button onClick={() => handleSetQty(item.id, item.qty - 1)}
                      ...
                      <div className="flex items-center gap-3">
                        <span className="font-display font-bold text-sm">{formatAr(item.subtotal)}</span>
                        <button onClick={() => handleRemove(item.id, item.product.name)}
          ...
          <aside className="lg:col-span-4 lg:sticky lg:top-32 self-start space-y-4">
            <div className="card-soft p-6">
              <h3 className="font-display font-bold text-base mb-4">Récapitulatif</h3>
              ...
              <div className="border-t border-border mt-4 pt-4 flex items-end justify-between">
                ...
                <span className="font-display font-bold text-2xl text-primary">{formatAr(calculateSubtotal())}</span>
              </div>

              <Button variant="hero" size="lg" className="w-full mt-5 group" onClick={handleOpenDevisModal}>
                Passer commande
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-background border border-border rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col animate-slide-up">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-border">
              <div>
                <h2 className="font-display font-bold text-lg text-foreground">Récapitulatif de commande</h2>
                <p className="text-xs text-muted-foreground mt-0.5">Vérifiez vos articles avant de confirmer</p>
              </div>
              <button onClick={() => setShowDevisModal(false)} className="p-2 rounded-lg hover:bg-secondary transition text-muted-foreground hover:text-foreground">
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <div className="flex items-center gap-0 px-6 pt-4 pb-2">
              {[{ n: 1, label: "Articles" }, { n: 2, label: "Options" }].map((step, i, arr) => (
                <div key={step.n} className="flex items-center gap-0 flex-1">
                  <div className="flex items-center gap-2">
                    <StepDot n={step.n} active={modalStep === step.n} done={modalStep > step.n} />
                    <span className={`text-xs font-medium ${modalStep === step.n ? 'text-foreground' : 'text-muted-foreground'}`}>{step.label}</span>
                  </div>
                ...
                <div className="space-y-4 pt-4">
                  ...
                  <div className="space-y-2">
                    {cartDetailed.map((item) => {
                      const imageUrl = getProductImageUrl(item.product);
                      return (
                        <div key={item.id} className="flex items-center gap-3 p-3 rounded-xl bg-secondary/20 border border-border/50">
                          ...
                          <p className="font-semibold text-sm text-primary tabular-nums whitespace-nowrap">
                            {formatPriceWithDevise(item.subtotal, devisForm.devise)}
                          </p>
                        </div>
                      );
                    })}
                  ...
                  {/* Devise */}
                  <div>
                    <label className="block text-xs font-medium text-muted-foreground mb-1.5 uppercase tracking-wider">Devise</label>
                    <div className="grid grid-cols-3 gap-2">
                      {[{ val: "MGA", label: "Ariary", flag: "🇲🇬" }].map(d => (
                        <button key={d.val}
                          onClick={() => setDevisForm(prev => ({ ...prev, devise: d.val }))}
                  ...
                  <div className="flex items-center justify-between p-4 bg-secondary/20 border border-border/50 rounded-xl">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-medium text-foreground">Livraison à domicile</p>
                        <p className="text-xs text-muted-foreground">+50 000 Ar</p>
                      </div>
                    </div>
                    <button
                    ...
                    >
                      <span className={`absolute top-0.5 w-5 h-5 rounded-full bg-white transition-transform shadow ${devisForm.besoinLivraison ? 'right-0.5' : 'left-0.5'}`} />
                    </button>
                  </div>
                </div>
              )}
              ...
              {modalStep === 2 && (
                <div className="space-y-5 pt-4">
                  ...
                  {devisForm.besoinLivraison && (
                    <div>
                      <label className="block text-xs font-medium text-muted-foreground mb-2 uppercase tracking-wider">Adresse de livraison</label>
                      {isLoadingAdresses ? (
                        ...
                        <div className="space-y-2 max-h-44 overflow-y-auto">
                          {adresses.map((adr) => (
                            <label key={adr.id} className={`flex items-start gap-3 p-3 border rounded-xl cursor-pointer transition-all
                              ${devisForm.adresseId === adr.id ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/30'}`}>
                              <input type="radio" name="adresse" checked={devisForm.adresseId === adr.id}
                                onChange={() => setDevisForm(prev => ({ ...prev, adresseId: adr.id, adressePersonnalisee: "" }))}
                                className="mt-1 accent-primary" />
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-1.5 mb-0.5">
                                  {adr.etiquette === 'Maison' && <Home className="h-3.5 w-3.5 text-green-500" />}
                                  {adr.etiquette === 'Appartement' && <Building className="h-3.5 w-3.5 text-blue-500" />}
                                  {adr.etiquette === 'Bureau' && <Package className="h-3.5 w-3.5 text-purple-500" />}
                                  <p className="font-medium text-sm text-foreground">{adr.nom_complet}</p>
                                  {adr.par_defaut_expedition && <span className="text-[10px] bg-primary/15 text-primary px-1.5 py-0.5 rounded-full">Par défaut</span>}
                                </div>
                      ...
                      <div className="mt-3">
                        <p className="text-xs text-muted-foreground mb-1.5">Ou saisissez une adresse :</p>
                        <textarea
                  ...
                  {/* Note */}
                  <div>
                    <label className="block text-xs font-medium text-muted-foreground mb-1.5 uppercase tracking-wider">Note (optionnelle)</label>
                    <textarea
                  ...
                  <div className="bg-secondary/20 border border-border/50 rounded-xl p-4">
                    ...
                    <div className="border-t border-border pt-2.5 mt-2 flex justify-between items-center">
                      ...
                      <span className="font-display font-bold text-xl text-primary">{formatPriceWithDevise(getTotalWithLivraison(), devisForm.devise)}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-3 p-3 bg-primary/5 border border-primary/20 rounded-xl">
                    ...
                    <p className="text-xs text-muted-foreground leading-relaxed">
                      Après confirmation, un responsable vous recontactera sous <strong className="text-foreground">24 à 48h</strong> pour valider votre commande. Aucun paiement ne sera demandé avant ce contact.
                    </p>
                  </div>
                </div>
              )}
            ...
            <div className="flex items-center justify-between gap-3 px-6 py-4 border-t border-border bg-secondary/5">
              ...
              <div className="flex items-center gap-2">
                ...
                {modalStep === 1 && (
                  <Button variant="hero" onClick={() => setModalStep(2)} className="flex items-center gap-2">
                    Continuer
                ...
                {modalStep === 2 && (
                  <>
                    ...
                    >
                      {isSubmitting && !devisValide ? <Loader2 className="h-4 w-4 animate-spin" /> : <Check className="h-4 w-4" />}
      ...
      {showHeartAnimation && (
        <div className="fixed inset-0 z-50 flex items-center justify-center pointer-events-none animate-slide-up">
          <div className="relative flex items-center justify-center">
            <img src={fosa} alt="Le Fosa" className="h-56 w-56 rounded-full shadow-2xl border-4 border-rose-400 animate-bounce-slow z-10 relative" />
            {[...Array(12)].map((_, i) => (
              <Heart key={i} className="absolute fill-rose-500 text-rose-500 animate-heart-orbit"
                style={{
                  width: `${16 + (i % 3) * 10}px`, height: `${16 + (i % 3) * 10}px`,
                  top: `${50 + 45 * Math.sin((i / 12) * 2 * Math.PI)}%`,
                  left: `${50 + 45 * Math.cos((i / 12) * 2 * Math.PI)}%`,
                  transform: 'translate(-50%, -50%)', animationDelay: `${i * 0.15}s`, opacity: 0.7 + (i % 3) * 0.1,
                }}
              />
            ))}
            {lastAddedProduct && (
              <div className="absolute -bottom-12 left-1/2 -translate-x-1/2 bg-background rounded-2xl px-4 py-2 shadow-lg border border-rose-200/30 whitespace-nowrap">
                <p className="text-sm font-medium text-rose-400">❤️ {lastAddedProduct} ajouté !</p>
              </div>
            )}
          </div>
        </div>
