...
import { Heart, ShoppingBag, Star, SlidersHorizontal, Search, Filter, Volume2, VolumeX, X, HelpCircle, Loader2, ChevronRight } from "lucide-react";
...
const Catalog = () => {
  ...
  const fetchAllProducts = async () => {
    try {
      ...
      let products = [];
      if (response.data.data) products = Array.isArray(response.data.data) ? response.data.data : [];
  ...
  const fetchFavorites = async () => {
    try {
      ...
      let favorisData = [];
      if (response.data.data) favorisData = Array.isArray(response.data.data) ? response.data.data : [];
      ...
      if (error.response?.status !== 401) console.error("Erreur chargement favoris:", error);
    }
  };
  ...
  const toggleFavorite = async (produitId: number, e?: React.MouseEvent) => {
    ...
    try {
      ...
      if (isCurrentlyFavorite) {
        ...
        toast({ title: "Retiré des favoris", description: "Produit retiré de votre liste" });
      } else {
        await api.post('/favoris', { produit_id: produitId });
        setFavorites([...favorites, produitId]);
        toast({ title: "Ajouté aux favoris", description: "Produit ajouté à votre liste" });
      }
    } catch (error: any) {
        ...
        <div className="container-x py-3">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <input
                type="range" min={1500000} max={15000000} step={500000} value={budget}
      ...
      {(searchNom || searchRef || searchCategory || searchSousCategory) && (
        <div className="container-x pt-4">
          <div className="flex flex-wrap items-center gap-2 p-2 bg-primary/5 border border-primary/20 rounded-lg">
            ...
            <div className="flex flex-wrap gap-1.5">
              {searchNom && (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-secondary text-foreground text-[10px] rounded-full">
                  Nom: {searchNom}
                  <button onClick={() => { const np = new URLSearchParams(searchParams); np.delete("nom"); setSearchParams(np); }} className="hover:text-red-500"><X className="h-2.5 w-2.5" /></button>
                </span>
              )}
              {searchRef && (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-secondary text-foreground text-[10px] rounded-full">
            ...
            <button onClick={() => setSearchParams({})} className="ml-auto text-[10px] text-muted-foreground hover:text-primary underline">Réinitialiser</button>
          </div>
        </div>
      )}
      ...
      <section className="container-x py-6">
          ...
          {showHelp && (
            <div className="card-soft p-2 max-w-md animate-fade-up">
              <div className="flex items-start gap-2 text-[10px]">
                ...
                <p className="text-muted-foreground">Utilise les filtres pour trouver ton bonheur ! Passe ta souris sur un produit pour que je te le présente !</p>
              </div>
            </div>
          )}
        </div>
        ...
        {isLoadingProducts ? (
          ...
          <div className="card-soft p-8 text-center">
            ...
            <p className="text-xs text-muted-foreground">Essayez d'élargir votre budget ou de changer de catégorie.</p>
          </div>
        ) : (
                  ...
                  <div className="p-2.5 space-y-1.5">
                    <div className="flex items-start justify-between gap-1">
                      <div className="flex-1 min-w-0">
                        <div className="text-[8px] font-mono uppercase text-primary/70">{p.reference?.split('-')[0] || p.categorie?.nom}</div>
                        <h3 className="font-semibold text-xs leading-tight truncate">{p.nom}</h3>
                      </div>
                    ...
                    <p className="text-[9px] text-muted-foreground line-clamp-2 leading-relaxed">{p.description_courte || p.tagline || 'Produit exceptionnel'}</p>
                    <div className="flex flex-wrap gap-1">
                      {productSpec(p, "processeur") && <span className="text-[8px] bg-secondary px-1.5 py-0.5 rounded-full">{productSpec(p, "processeur")?.slice(0, 12)}</span>}
                      {productSpec(p, "carte_graphique") && <span className="text-[8px] bg-secondary px-1.5 py-0.5 rounded-full">{productSpec(p, "carte_graphique")?.slice(0, 12)}</span>}
                    </div>
